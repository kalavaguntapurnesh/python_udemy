...
# JSON flattening is the process of converting a nested JSON object into a single-level object by combining nested keys into one key path.
...
def flatten_json(data, parent_key='', sep='.'):
    ...
    if isinstance(data, dict):
        for k, v in data.items():
            ...
            print(full_key)
            items.update(flatten_json(v, full_key, sep=sep)) # will keep on calling the values until flattening is done
